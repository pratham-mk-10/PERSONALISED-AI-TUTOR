  ...
  return (
    <AnimationPlayer duration={12000} title="Fix the mistake">
      ...
      {({ progress }) => {
        ...
        const hitProgress = 0.3;
        let step = 0;

        if (progress < 0.45) step = 0;       // show wrong (after ray hits)
        else if (progress < 0.7) step = 1;   // highlight wrong
        ...
        const incPhase = clamp(progress / hitProgress, 0, 1);
        const rayHasHitMirror = incPhase >= 1;
